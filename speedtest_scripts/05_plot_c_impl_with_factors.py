...
from c_pthread import train, predict
from _shared_func import run_speedtest_and_plot_graph, load_all_datasets

NUMBER_OF_FOLDS = int(os.environ.get("NUMBER_OF_FOLDS", 5))
DATASET_LEN_INCREMENT = int(os.environ.get("DATASET_LEN_INCREMENT", NUMBER_OF_FOLDS * 3))
...
if __name__ == "__main__":
    ...
    x_total = [
        [x + i * random.uniform(-0.1, 0.1) for i in range(FEATURE_GROWING_FACTOR) for x in x_elem]
    ...
    run_speedtest_and_plot_graph(
        f"images/graphs/speedtest_plots/c_impl_{NUMBER_OF_FOLDS}fold_{len(x_total)}d_{len(x_total[0])}f.png",
        x_total,
        y_total,
        NUMBER_OF_FOLDS,
